...
| ------------------------------------------------------------------------------------------ | ----------- | ----- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Preconditions](adaptive-harness/research-grounding.md#required-pre-phase-0-preconditions) | In progress | Codex | -         | `npm run check`, `npm run build`, `git diff --check`; `src/provider/openai-compatible.test.ts`; `src/agent/loop.test.ts`; `src/agent/compact.test.ts`; `src/agents/manager.test.ts`; `src/session/agent-session.test.ts`; `src/session/run-accounting.test.ts`; `src/session/run-ambient.test.ts`; `src/book-home.test.ts`; `src/settings-loader.test.ts`; `src/commands/loader.test.ts`; `src/headless.test.ts`; `src/harness/evaluation/runner.test.ts`; `scripts/edit-eval.test.ts`; `scripts/edit-eval-settings.test.ts`; `scripts/edit-eval-worker.test.ts`; `scripts/check-architecture.test.ts` | Terminal framing now distinguishes `[DONE]`, EOF-after-`finish_reason`, and interrupted EOF. Per-request root/child/resume attribution, versioned pricing, provider identity, direct/root-inclusive accounting, and fail-closed budget checks include compaction calls. Versioned ambient snapshots freeze effective model overrides, initial permission mode, secret-safe settings, tool-contract, memory, policy, runtime, Book-home-path, and `agents.mode` fingerprints for root and managed-child runs. The trusted built-in evaluator foundation provisions a disposable workspace, `BOOK_HOME`, user-config/cache/temp roots, a sanitized allowlist-based environment, bounded output, explicit process terminal status, and bounded graceful/forced process-tree teardown; architecture checks enforce the initial live-runtime/evaluator/trusted-kernel dependency rules. The edit-reliability evaluation now executes each trial through this boundary with agents disabled and generated isolated settings that preserve the resolved provider-facing model ID, model metadata, retry policy, and provider-option explicitness. The runner is a reproducibility boundary, not a Windows security sandbox; compact/skill eval integration remains open. Accounting and ambient snapshots remain explicitly partial when provider usage is missing or command/skill/MCP registries, random seed, repository/runtime revisions, and isolated Book-home contents are not controlled. Tier B integration and Tier C trust/permission/container-isolation gates remain open. |
