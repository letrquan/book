...
- **Tier B calibration plumbing is verified for the current built-in evaluator entry points.** Edit,
  compaction, and offline skill-observation evaluation run in disposable external processes with
  runner-owned date, seed, runtime revision, materialized-fixture revision, sanitized environment,
  bounded output, and bounded process-tree teardown. These are reproducibility controls only. They
  are not a sandbox or provider/promotion authority; contract-v2 requires a registered packaged
  worker, provider broker, and host-attested enforcement before such authority exists.
...
- **Phase 0 contract-v1 was superseded by decisions #46–#49.** Contract-v2 and its calibration
  assets are verified. The current corpus is calibration-only; no provider-grade worker/broker
  authority, populated confirmatory split, or promotion evidence exists. This does not activate
  adaptive behavior or broaden the trust boundary. Tier C remains blocked.
...
| ----- | ----------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| 0     | Verified    | [Evaluation contract and fixtures](adaptive-harness/phase-0-evaluation-contract.md)    | Improvement can be falsified with stable, non-promotional metrics             |
| 1     | Verified    | [Contracts and disabled boundary](adaptive-harness/phase-1-contracts-boundary.md)      | Harness is inert and runtime-equivalent when off             |
| 2     | Verified    | [Run evidence ledger](adaptive-harness/phase-2-run-evidence-ledger.md)                 | Runs are explainable without behavior changes                |
| 3     | Verified    | [Validated workflow registry](adaptive-harness/phase-3-workflow-registry.md)           | Fixed workflows are safe and reproducible                    |
...
| ------------------------------------------------------------------------------------------------- | ----------- | ----- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Preconditions Tier A/B](adaptive-harness/research-grounding.md#2026-08-05-readiness-disposition) | Verified    | Codex | -         | `npm run check`, `npm run build`, `git diff --check`; provider/loop/compact/session/accounting/ambient/headless/runner/eligibility tests; edit/compact/skill evaluator tests; architecture checks | Verified only for evaluator-controlled, trusted built-in, single-agent evaluation. Terminal outcomes, root/child/resume attribution, compaction and retry accounting, versioned pricing, verified model identity, complete ambient schema v2, isolated captured Book home, dirty/untracked runtime identity, materialized-fixture identity, runner-owned date/seed, bounded process isolation, and fail-closed eligibility are covered. The dedicated Phase 0 change consumes this readiness without broadening Tier C. |
...
| [0. Evaluation contract](adaptive-harness/phase-0-evaluation-contract.md)                         | Verified    | Codex | -         | `npm run check`: format, lint, strict types, architecture, unit (185 files; 1,858 passed, 5 skipped), contract (4 files; 31 passed); focused evaluator (6 files; 50 passed); targeted lint; build; Draft 2020-12 schema validation; `git diff --check` | Supersedes v1 under #46–#49. `calibration-public-v1` is explicitly non-promotional; the confirmatory corpus is an empty contract/template with no fabricated membership. Raw/block accounting, fixed horizons, A/A noise, per-guardrail power, multiplicity/query spending, execution diagnostics, compatibility roles, authenticated attestations, approval, and review reliability fail closed. Integration diagnostics passed 6/7 files (70 passed, 7 skipped); 13 known host-sensitive TUI startup timeouts are non-authoritative and do not exercise Phase 0. Provider-grade worker/broker authority remains unavailable without host enforcement; Tier C stays blocked. |
| [1. Contracts and disabled boundary](adaptive-harness/phase-1-contracts-boundary.md)              | Verified    | Codex | -         | Format/lint/type/architecture gates; unit (1,800 passing plus isolated retry of one unrelated timing test, 5 skipped), contract (31 passing), focused Phase 1 tests, CLI settings integration (4 passing), build, `git diff --check`; clean integration baseline (82 passing, 7 skipped) | Verified with `harness.mode = off` as the only available mode. Future modes fail before migrations, settings locks, storage, or runtime setup; root and managed-agent runtimes remain contracts-only. Observer outcomes and bounded event text are explicit. The disabled coordinator creates no harness run ID, context, observer state, or storage; provider messages/tools remain identical when omitted versus explicitly off. Phase 2 may add observation separately; Tier C remains blocked. |
| [2. Observation ledger](adaptive-harness/phase-2-run-evidence-ledger.md)                          | Verified    | Codex | 2026-08-13 | `npm run check`: format, lint (0 warnings), strict types, architecture, unit (199 files; 1,941 passed, 5 skipped), contract (4 files; 31 passed); `npm run build`; focused harness suites (39 passed across 6 files) plus session harness (7) and headless harness (2); `git diff --check` clean. Integration: 6/7 files passed (81 passed, 7 skipped); the 2 failures are host-sensitive TUI startup-fire timeouts in `src/tui/tui-integration.test.ts`, shown non-causal by A/B re-runs — the same file fails 12 and 16 of 22 at `HEAD` with the Phase 2 TUI change reverted, versus 2-18 with it applied. They exercise no harness code (`harness.mode` is `off` there) and are non-authoritative on this host. | Verified against closed decisions [#50](https://github.com/letrquan/book/issues/50) and [#51](https://github.com/letrquan/book/issues/51). Observe is the only enabled mode. Every root request gets a fresh root run identity; resume/child relationships are explicit. Evidence is a per-root single-writer canonical JSONL hash chain with explicit durability and seal, rebuildable indexes, signed retention pins, allowlisted redacted metadata/references, and fail-closed completeness — directory-sync is honestly reported `unavailable`, so every seal stays `evidenceEligibility: ineligible` and no observe evidence is promotion-eligible. Off remains inert; Tier C and all selecting/learning modes remain blocked. |
| [3. Fixed workflow registry](adaptive-harness/phase-3-workflow-registry.md)                       | Verified    | Claude | 2026-08-14 | `npm run check`: format, lint (0 warnings), strict types, architecture, unit (207 files; 2,065 passed, 5 skipped), contract (6 files; 37 passed); `npm run test:integration` (7 files; 83 passed, 9 skipped, fully green on this host); `npm run build`; focused harness suites (14 files; 134 passed) and `settings-cli` (6 passed). | Built-in registry only; `.book/harness/workflows/*.json` deferred, and workflow IDs are opaque keys that cannot address a path or the candidate store. No new harness mode: selection is effective under `observe` and fails closed under `off`, on unknown IDs, and on path-like IDs at `book config set`, `loadConfig`, the CLI flag, and `AgentSession.run()`. A run with no selection keeps Phase 2 `baseline`. No field reports enforcement — every disposition is guidance, clamped, or host-owned — and clamps are recorded as `capability_clamped` evidence. `minimal` leaves provider messages byte-identical to a no-harness run. Session overrides are process-scoped and do not survive resume; `/harness` and SDK fields deferred. Evidence stays promotion-ineligible (directory sync `unavailable`); Tier C remains blocked. |
