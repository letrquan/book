...
| Milestone 5: Shared `AgentSession` | Partial | Added a stable public `AgentEvent` contract plus immutable session snapshot/reducer types. `AgentSession` now owns send preflight checkpointing, input expansion/provenance, user timeline persistence, agent-loop invocation, stale-run-safe authoritative snapshots, shared event emission, interaction settlement, cancellation/reset composition, and send lease completion. `useAgent` projects terminal completion/error from session snapshots but still owns optimistic transcript projection, UI cleanup, compaction persistence, and host lifecycle transitions. |
...
- `npm run check` passes: formatting, lint, typecheck, architecture checks, 1,062 unit tests,
...
1. Move compaction orchestration and boundary persistence behind the session contract.
2. Move session create/resume/clear/end lifecycle behind the session contract.
