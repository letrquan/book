...
**Tech stack**: TypeScript (ES2022/ESM, `moduleResolution: bundler`), React 19 via Ink 6.8.0 for the TUI, tsup for building, Vitest 4 for testing, Zod 4 for config validation. Node.js 22.19+.
...
New features are built by running Book, not by hand-editing `src/`, and no change is done until it has been watched in the real TUI. The mandate and its toolkit (PTY driver, mock provider, smoke test) live in the `run-book` skill — `.claude/skills/run-book/SKILL.md`.
