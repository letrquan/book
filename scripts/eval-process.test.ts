...
describe('runEvaluationProcess', () => {
  ...
  it('terminates evaluator descendants before returning a timeout', async () => {
    ...
    const grandchildSource = [
      "const fs = require('node:fs');",
      // Comfortably after the kill, so a surviving grandchild is proof of a
      // leak rather than of the write racing termination.
      "setTimeout(() => fs.writeFileSync(process.env.SURVIVOR_PATH, 'alive'), 3000);",
      'setInterval(() => {}, 1000);',
    ].join('');
    const parentSource = [
      "const fs = require('node:fs');",
    ...
    try {
      ...
      const result = await runEvaluationProcess({
        command: process.execPath,
        args: ['-e', parentSource],
        // Deliberately generous. What this pins is that descendants die, not
        // how fast: under a loaded full-suite run a 300ms deadline and a 2s
        // bound both expired on scheduling alone, which failed the test while
