...
Explorer runs in the parent workspace with a hard read-only capability boundary and does not require Git, snapshots, or worktrees. Patcher and validator runs retain synthetic Git snapshots and isolated worktrees under `~/.book/worktrees/<repo-hash>/<agent-id>`, with state and transcripts under `~/.book/agents/<repo-hash>/`. Parent-facing `AgentList`, `AgentGet`, `AgentWait`, `AgentSpawn`, `AgentSend`, and `AgentStop` results contain summaries/evidence IDs only; the TUI and SDK host can inspect detailed transcripts separately. A patcher commit cannot be applied until a distinct validator passes the exact candidate commit.

Child completion is delivered automatically to the correct parent session as a compact agent-update card and a persisted provider-facing notification; `AgentWait` is only an explicit synchronization barrier. Delivery acknowledgements survive restart, terminal rows freeze their duration and final preview, and lifecycle tool rows show semantic actions instead of serialized JSON prefixes.
...
Running children appear in a task panel directly below the prompt. From an empty prompt, press Down to select the first child and Enter to open its transcript; `/tasks` remains available for explicit management. In the focused panel, Up/Down selects `main` or a child, `x` stops or hides a child, and Esc returns to the main prompt. Opening a child keeps its input available while the parent is generating, so normal prompts become direct follow-ups; completed children resume under the same identity through lower-level commands. Completed and failed rows remain only until their report is accepted by the parent, then clean themselves from the panel automatically. User-stopped runs leave immediately.
...
Built-ins include session controls (`/clear`, `/resume`, `/compact`, `/rewind`), managed-agent controls (`/agents`, `/agent`), config (`/model`, `/providers`, `/effort [low|medium|high|xhigh|max]`, `/config`, `/permissions`, `/theme`), inspection (`/status`, `/cost`, `/usage`, `/context`, `/diff`, `/skills`, `/memory`), and agent prompts (`/init`, `/review`, `/security-review`). `/model` switches models, while `/providers` opens the same picker for provider management. Workspace-local BYOK providers are labeled `[local BYOK]`; select one of their models and press `Alt+D` to remove the provider, its credentials, and its saved model catalog from `.book/settings.local.json`. `/effort` opens a picker when called without an argument and saves successful selections to `.book/settings.local.json`.
