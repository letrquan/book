...
import type { AgentConfig, CompactBoundary, ConversationCheckpointV2, Message } from './types.js';
...
describe('runHeadless — split session projections', () => {
  it('returns the full transcript while sending only active context to the provider', async () => {
    const oldUser: Message = {
      id: 'old-user',
      role: 'user',
      content: 'OLD TRANSCRIPT MARKER',
    ...
    const result = await runHeadless(config, createDefaultRegistry(), {
      ...
      outputFormat: 'text',
      history: [recentUser],
      transcript: [oldUser, recentUser],
      contextHistory: [recentUser],
    ...
    expect(result.messages.map((message) => message.content)).toContain('OLD TRANSCRIPT MARKER');
    expect(result.messages.map((message) => message.content)).toContain('NEW PROMPT');
    expect(requestBody).toContain('RECENT CONTEXT MARKER');
    expect(requestBody).toContain('NEW PROMPT');
    expect(requestBody).not.toContain('OLD TRANSCRIPT MARKER');
  });

  it('exposes current-session history tools only for persisted sessions', async () => {
    const sessions = new SessionStore(makeWorkspace());
    const sessionId = sessions.create({ cwd: config.workspace });
    sessions.append(sessionId, {
      ...
      eventId: 'old-user',
      timestamp: 1,
      data: { content: 'recoverable evidence' },
    });
    let requestBody = '';
    vi.stubGlobal(
      'fetch',
    ...
    await runHeadless(config, createDefaultRegistry(), {
      prompt: 'search old evidence',
      inputFormat: 'text',
      outputFormat: 'text',
      history: sessions.load(sessionId).contextHistory,
      mode: 'bypassPermissions',
      stdout: { write: () => true },
      sessionStore: sessions,
      sessionId,
    ...
    expect(requestBody).toContain('SessionHistorySearch');
    expect(requestBody).toContain('SessionHistoryRead');
  });

  it('resumes a v2 checkpoint with live file freshness materialized', async () => {
    const workspace = makeWorkspace();
    writeFileSync(join(workspace, 'a.ts'), 'export const value = 2;\n', 'utf-8');
    const checkpoint: ConversationCheckpointV2 = {
      version: 2,
      generation: 1,
      throughEventRef: 'session://current/event/old-user',
      stateAtCheckpoint: {
        taskSummary: 'Earlier work.',
        status: 'paused',
        sourceRefs: ['session://current/event/old-user'],
      },
      constraints: [],
      files: [
        {
    ...
    await runHeadless(freshConfig({ workspace }), createDefaultRegistry(), {
      ...
      contextHistory: [checkpointMessage],
      transcript: [],
      mode: 'bypassPermissions',
      stdout: { write: () => true },
      persistSession: false,
    ...
    expect(requestBody).toContain('stale locator');
    expect(requestBody).toContain('different workspace');
  });
});
