...
import { detectMemoryCandidate, maybeCaptureMemoryCandidate, shouldRejectMemoryText } from './memory-autosave.js';
...
describe('detectMemoryCandidate', () => {
  it('captures explicit remember statements as user candidates', () => {
    const candidate = detectMemoryCandidate({ userMessage: 'Remember that I prefer concise final summaries.' });
  ...
  it('classifies project conventions', () => {
    const candidate = detectMemoryCandidate({ userMessage: 'Remember that in this repo we use pnpm.' });
  ...
  it('captures useful confirmations only with previous assistant context', () => {
    ...
    const candidate = detectMemoryCandidate({ userMessage: 'that worked', previousAssistant: 'Try using the portable temp dir.' });
  ...
  it('rejects secret-looking text', () => {
    expect(shouldRejectMemoryText('api_key=sk-abcdefghijklmnopqrstuvwxyz123456')).toContain('secret');
    expect(detectMemoryCandidate({ userMessage: 'Remember that api_key=sk-abcdefghijklmnopqrstuvwxyz123456' })).toBeNull();
  });
});
