...
const frameUpdateListeners = new Set<() => void>();

function notifyFrameUpdate(): void {
  for (const listener of frameUpdateListeners) {
    try {
      listener();
    } catch {
      // A terminal overlay must never break Ink's renderer.
    }
  }
}
...
/** Subscribe to completed Ink frame writes, used to preserve terminal overlays. */
export function subscribeToFrameUpdates(listener: () => void): () => void {
  frameUpdateListeners.add(listener);
  return () => frameUpdateListeners.delete(listener);
}
...
export function createFrameCapturingRenderer(
  ...
  const render = ((output: string) => {
    latestFrame = output;
    const rendered = base(output);
    if (rendered !== false) notifyFrameUpdate();
    return rendered;
