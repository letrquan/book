...
export async function runAgentLoop(
  ...
  while (config.maxTurns == null || config.maxTurns <= 0 || turn < config.maxTurns) {
    ...
    if (streamError) {
      if (isContextOverflowError(streamError) && !overflowRecoveryAttempted) {
        overflowRecoveryAttempted = true;
        const beforeTokens = estimateHistoryTokens(newHistory);
        let compacted = false;
        if (config.autoCompactEnabled !== false && callbacks.onCompact) {
          const estimatedUsage: Usage = {
            promptTokens: requestTokens,
            completionTokens: 0,
            totalTokens: requestTokens,
        ...
        const afterTokens = estimateHistoryTokens(newHistory);
