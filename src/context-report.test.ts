...
function msg(role: 'user' | 'assistant', content: string, toolCalls = 0, toolResults = 0): Message {
  ...
  if (toolCalls) m.toolCalls = Array.from({ length: toolCalls }, () => ({ id: 't', name: 'Read' }) as never);
  if (toolResults)
    m.toolResults = Array.from({ length: toolResults }, () => ({ toolCallId: 'x', success: true, output: 'x'.repeat(40) }) as never);
  return m;
}
...
describe('buildContextReport', () => {
  it('produces a human-readable report with budget percentage', () => {
    ...
    expect(report).toContain('not loaded yet (Phase 1b');
  });
});
