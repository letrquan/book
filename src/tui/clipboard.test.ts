...
describe('clipboard', () => {
  ...
  it('reports an OSC 52 write as unconfirmed when platform copy is unavailable', async () => {
    const spawnFn = (() => {
  ...
  it('reports failure when neither OSC 52 nor a platform command succeeds', async () => {
    const spawnFn = (() => {
      throw new Error('ENOENT');
    }) as unknown as ClipboardWriterDeps['spawn'];
    await expect(
