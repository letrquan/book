...
describe('ChatPanel Ink rendering', () => {
  ...
  it('renders tool calls and results under the assistant turn that produced them', () => {
    ...
    expect(output.indexOf('[OK] Read src/a.ts')).toBeGreaterThan(output.indexOf('I will inspect it.'));
    expect(output.indexOf('[OK] Read src/a.ts')).toBeLessThan(output.indexOf('The file looks good.'));
  });
  ...
  it('renders retry and stall labels on the active streaming assistant message', () => {
    const stalled = render(withTheme(
      <AgentMessage
    ...
    expect(frame(stalled.lastFrame)).toContain('Retrying: Waiting for API response · will retry in 20s · check your network');
    stalled.unmount();

    const retrying = render(withTheme(
      <AgentMessage
  ...
  it('keeps the streaming tail visible only while auto-scroll is enabled', () => {
    ...
    expect(frame(view.lastFrame)).toContain('message 29');
