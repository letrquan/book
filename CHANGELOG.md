...
- **A run stopped by refused web calls names what was refused** (#246). The `all_tools_blocked`
  ...
  refusal codes from one place. A cross-origin redirect is now reported before its target is judged
  or resolved, so a redirect to a private or plain-HTTP host is no longer refused under the
  target's code (naming a host the model never asked for), and the TUI row says why the redirect
  stopped; RFC 9637's documentation prefix `3fff::/20` is refused beside `2001:db8::/32`.
